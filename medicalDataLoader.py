...
def make_dataset(root, mode):
    assert mode in ['train','train_unlabeled', 'val', 'test']
    items = []
    ...
    if mode == 'train':
        ...
        unlabeled_mask_path = os.path.join(root, 'train', 'Img-Unlabeled')

        images = os.listdir(unlabeled_img_path)
        labels = os.listdir(unlabeled_mask_path)

        images.sort()
        labels.sort()

        for it_im, it_gt in zip(images, labels):
            item = (os.path.join(unlabeled_img_path, it_im), os.path.join(unlabeled_mask_path, it_gt))
            items.append(item)
